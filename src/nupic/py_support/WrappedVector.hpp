...
template <typename T> inline std::string tts(const T x) {
  std::ostringstream s;
  s << x;
  return s.str();
}
...
public:
  ...
  WrappedVectorIter<T> slice(int i, int j) const {
    if (j >= i)
      return WrappedVectorIter(j - i, incr_, p_ + i * incr_);
    else
      return WrappedVectorIter(i - j, -incr_, p_ + i * incr_);
  }

  WrappedVectorIter<T> slice(int start, int /*stop*/, int step,
                             int length) const {
  ...
  WrappedVectorIter<T> operator+(size_t n) const {
    return slice(int(n), n_ - int(n));
  }
  /// Advance in-place
  WrappedVectorIter<T> &operator+=(int n) {
    *this = slice(n, n_ - n);
    return *this;
  }
  WrappedVectorIter<T> &operator+=(size_t n) {
    *this = slice(int(n), n_ - int(n));
    return *this;
  }
  /// Prefix increment
  WrappedVectorIter<T> &operator++() {
    *this = slice(1, n_ - 1);
    return *this;
  }
  /// Postfix increment
  WrappedVectorIter<T> operator++(int) {
    WrappedVectorIter<T> a = *this;
    *this = slice(1, n_ - 1);
    return a;
  }
  /// Move back
  ...
  /// Postfix decrement
  WrappedVectorIter<T> operator--(int) {
    WrappedVectorIter<T> a = slice(-1, n_ + 1);
    *this = slice(-1, n_ + 1);
    return a;
  }
  /// Difference between two iterators
  int operator-(const WrappedVectorIter<T> &i) const {
    return int((p_ - i.p_) / incr_);
  }
  ...
  template <typename T2> void copyFrom(int dim, int incr, const T2 *in) {
  ...
  template <typename T2> void into(int dim, int incr, T2 *out) const {
    const int n = n_, inc1 = incr_, inc2 = incr;
    const T *p1 = p_;
    T2 *p2 = out;
    for (int i = 0; i < n; ++i) {
...
public:
  ...
  //  WrappedVector(const nupic::Belief &b) : p_(b.size(), 1,
  //  const_cast<nupic::Real *>(b.ptr())), own_(0) {}
  WrappedVector(const std::vector<nupic::Real> &v)
      : p_(int(v.size()), 1, const_cast<nupic::Real *>(&(v[0]))), own_(0) {}
  WrappedVector(const WrappedVector &v) : p_(v.p_), own_(0) {}
  WrappedVector &operator=(const WrappedVector &v) {
    this->free();
    p_ = v.p_;
    own_ = 0;
    return *this;
  ...
  void setPointer(int n, int incr, nupic::Real *p) {
    this->free();
    p_.n_ = n;
    p_.incr_ = incr;
    p_.p_ = p;
  }
  void setPointer(int n, nupic::Real *p) {
    this->free();
    p_.n_ = n;
    p_.incr_ = 1;
    p_.p_ = p;
  ...
  template <typename T> void adjust(T &endPoint) const {
    T n = (T)p_.size();
    if (endPoint < 0)
      endPoint += n;
    else if (endPoint > n)
  ...
  WrappedVector slice(int i, int j) const { return p_.slice(i, j); }
  ...
  void __iadd__(const WrappedVector &v) {
    if (!(p_.n_ == v.p_.n_))
      throw std::invalid_argument("Sizes must match: " + tts(p_.n_) + " " +
                                  tts(v.p_.n_));
  ...
  template <typename T2> void copyFromT(int n, int incr, const T2 *p) {
    if (!(p_.n_ == n))
      throw std::invalid_argument("Sizes must match: " + tts(p_.n_) + " " +
                                  tts(n));
  ...
  template <typename T2> void copyIntoT(int n, int incr, T2 *p) const {
    p_.into(n, incr, p);
  }
