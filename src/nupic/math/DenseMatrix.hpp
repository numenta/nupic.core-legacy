...
/**
 */
template <typename Int, typename Float> struct Dense {
  ...
  inline const_iterator begin(const Int i) const {
    return m.begin() + i * ncols;
  }
  ...
  template <typename InIter> inline void addRow(InIter begin) {
  ...
  void clear() { std::fill(m.begin(), m.end(), Real(0)); }
  ...
  bool isRowZero(Int row) const { return nNonZerosOnRow(row) == 0; }

  bool isColZero(Int col) const { return nNonZerosOnCol(col) == 0; }
  ...
  template <typename OutIter> void nNonZerosPerRow(OutIter it) const {
    ITER_1(nrows)
    *it++ = nNonZerosOnRow(i);
  }

  template <typename OutIter> void nNonZerosPerCol(OutIter it) const {
  ...
  template <typename InIter> Float rowL0Dist(Int row, InIter x) const {
    Float val = 0;
    for (Int j = 0; j != ncols; ++j)
  ...
  template <typename InIter> Float rowLMaxDist(Int row, InIter x) const {
    Float val = 0;
    for (Int j = 0; j != ncols; ++j)
  ...
  template <typename InIter, typename OutIter>
  inline void LpNearest(Float p, InIter x, OutIter nn, Int k = 1,
    ...
    std::partial_sort(
        ...
        predicate_compose<std::less<Float>,
                          nupic::select2nd<std::pair<Int, Float>>>());
  ...
  template <typename InIter, typename OutIter>
  inline void L0Nearest(InIter x, OutIter nn, Int k = 1,
                        bool take_root = false) const {
    std::vector<std::pair<Int, Float>> dists(nrows);
  ...
  template <typename InIter, typename OutIter>
  inline void LMaxNearest(InIter x, OutIter nn, Int k = 1,
                          bool take_root = false) const {
    std::vector<std::pair<Int, Float>> dists(nrows);
  ...
  template <typename InIter> std::pair<Int, Float> dotNearest(InIter x) const {
  ...
  void apply(const Dense<Int, Float> &B, Dense<Int, Float> &C,
             binary_functor f) {
    ITER_2(nrows, ncols)
  ...
  inline void setZero(Int i, Int j) { at(i, j) = 0; }

  inline void setNonZero(Int i, Int j, const Float &val) { at(i, j) = val; }

  inline void set(Int i, Int j, const Float &val) { at(i, j) = val; }

  template <typename InIter> inline void add(Int row, InIter x) {
    for (Int j = 0; j < ncols; ++j)
      at(row, j) += *x++;
  }
