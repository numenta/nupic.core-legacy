 ...
 */
template <typename Index, typename Value> class SparseRLEMatrix {
public:
...
public:
  //--------------------------------------------------------------------------------
  inline SparseRLEMatrix() : compress_(false), data_(), indb_(), nzb_(), c_() {}

  //--------------------------------------------------------------------------------
  inline SparseRLEMatrix(std::istream &inStream)
      : compress_(false), data_(), indb_(), nzb_(), c_() {
    fromCSR(inStream);
  }
  ...
  //--------------------------------------------------------------------------------
  inline bool isCompressed() const { return compress_; }

  //--------------------------------------------------------------------------------
  inline ulong_size_type nRows() const { return data_.size(); }

  //--------------------------------------------------------------------------------
  inline size_type nCols() const { return indb_.size(); }
   ...
   */
  inline void compact() {
    if (capacity() == nNonZeros() && data_.capacity() == data_.size() &&
   ...
   */
  template <typename InputIterator>
  inline void appendRow(InputIterator begin, InputIterator end) {
    { // Pre-conditions
      NTA_ASSERT(begin <= end) << "SparseRLEMatrix::appendRow: "
                               << "Invalid range";
    } // End pre-conditions
  ...
  inline void getRowToDense(ulong_size_type r, OutputIterator begin,
                            OutputIterator end) const {
    { // Pre-conditions
      NTA_ASSERT(0 <= r && r < nRows()) << "SparseRLEMatrix::getRow: "
                                        << "Invalid row index: " << r;
  ...
  //--------------------------------------------------------------------------------
  /**
   * Returns index of first row within <distance> of argument, or nRows() if
   * none.
   */
  template <typename InputIterator>
  inline ulong_size_type firstRowCloserThan(InputIterator begin,
                                            InputIterator end,
                                            nupic::Real32 distance) const {
  ...
  inline void toCSR(std::ostream &outStream) const {
    { // Pre-conditions
      NTA_ASSERT(outStream.good()) << "SparseRLEMatrix::toCSR: Bad stream";
    } // End pre-conditions

    outStream << getVersion() << " " << nRows() << " " << nCols() << " "
              << (compress_ ? "1" : "0") << " " << std::setprecision(15);
  ...
  inline void fromCSR(std::istream &inStream) {
    { // Pre-conditions
      NTA_ASSERT(inStream.good()) << "SparseRLEMatrix::fromCSR: Bad stream";
    } // End pre-conditions
    ...
    NTA_CHECK(version == getVersion())
        << "SparseRLEMatrix::fromCSR: Unknown version: " << version;
    ...
    for (ulong_size_type i = 0; i != nrows; ++i) {
      ...
      */
      ...
      for (size_type j = 0; j != data_[i].first.size(); ++j) {
        ...
        NTA_CHECK(idx <= nCols()) << "SparseRLEMatrix::fromCSR: "
                                  << "Invalid index: " << idx;
        if (1 < j)
  ...
  inline void print(std::ostream &outStream) const {
    { // Pre-conditions
      NTA_CHECK(outStream.good()) << "SparseRLEMatrix::print: Bad stream";
    } // End pre-conditions
  ...
  inline void debugPrint() const {
    std::cout << "n rows= " << nRows() << " n cols= " << nCols()
              << " n nz= " << nNonZeros() << " n bytes= " << nBytes()
              << std::endl;
    std::cout << "this= " << sizeof(SparseRLEMatrix) << " Row= " << sizeof(Row)
              << " size= " << sizeof(size_type)
              << " value= " << sizeof(value_type) << " uLong= " << sizeof(uLong)
              << std::endl;
    std::cout << "data= " << data_.capacity() << " " << data_.size()
              << std::endl;
    std::cout << "indb= " << indb_.capacity() << " " << indb_.size()
              << std::endl;
    std::cout << "nzb= " << nzb_.capacity() << " " << nzb_.size() << std::endl;
    std::cout << "c= " << c_.capacity() << " " << c_.size() << std::endl;
    for (ulong_size_type i = 0; i != nRows(); ++i)
      std::cout << "row " << i << ": first: " << data_[i].first.capacity()
                << " " << data_[i].first.size()
                << " second: " << data_[i].second.capacity() << " "
...
private:
  ...
  inline void compressRow_(ulong_size_type row) {
    { // Pre-conditions
      NTA_ASSERT(0 <= row && row < nRows()) << "SparseRLEMatrix::compressRow_: "
                                            << "Invalid row index: " << row;
    } // End pre-conditions
