...
public:
  ...
  PyTensorIndex(nupic::UInt32 i) : rank_(1) { index_[0] = i; }
  ...
  PyTensorIndex(nupic::UInt32 i, nupic::UInt32 j, nupic::UInt32 k,
                nupic::UInt32 l)
      : rank_(4) {
    index_[0] = i;
    index_[1] = j;
  ...
  PyTensorIndex(nupic::UInt32 i, nupic::UInt32 j, nupic::UInt32 k,
                nupic::UInt32 l, nupic::UInt32 m, nupic::UInt32 n)
      : rank_(6) {
    index_[0] = i;
    index_[1] = j;
    index_[2] = k;
  ...
  PyTensorIndex(const TIV &i) : rank_(i.size()) {
    if (rank_ > maxRank) {
      char errBuf[512];
      snprintf(errBuf, 512,
               "Tensors may not be constructed of rank greater than %d.",
               maxRank);
      rank_ = 0;
  ...
  template <typename T> PyTensorIndex(int nd, const T *d) : rank_(nd) {
    if (nd > maxRank) {
      char errBuf[512];
      snprintf(errBuf, 512,
               "Tensors may not be constructed of rank greater than %d.",
    ...
    if (d)
      std::copy(d, d + nd, index_);
    else
      std::fill(index_, index_ + nd, 0);
  }
  ...
  nupic::UInt32 __getitem__(int i) const {
    if (i < 0)
      i += rank_;
    return index_[i];
  }
  void __setitem__(int i, nupic::UInt32 d) {
    if (i < 0)
      i += rank_;
    index_[i] = d;
  }
  ...
  TIV asTuple() const { return TIV(index_, index_ + rank_); }

  TIV __getstate__() const { return asTuple(); }
};

//--------------------------------------------------------------------------------
inline PyTensorIndex concatenate(const PyTensorIndex &i1,
                                 const PyTensorIndex &i2) {
...
public:
  PyDomain(const TIV &lowerHalfSpace)
      : nupic::Domain<nupic::UInt32>(lowerHalfSpace) {}
  ...
  PyTensorIndex getSliceBounds() const {
    ...
    for (nupic::UInt32 i = 0; i < n; ++i) {
      nupic::DimRange<nupic::UInt32> r = (*this)[i];
      if (!(r.getDim() == i))
        throw std::invalid_argument("Out-of-order dims.");
      if (r.empty()) {
      } else {
  ...
  bool doesInclude(const TIV &x) const { return includes(x); }
...
public:
  ...
  nupic::UInt32 getBound(const nupic::UInt32 dim) const {
    return tensor_.getBound(dim);
  }
  ...
  void resize(const TIV &dims) { tensor_.resize(PyTensorIndex(dims)); }

  void resize(const PyTensorIndex &dims) { tensor_.resize(dims); }
  ...
  void setZero(const PyDomain &range) { tensor_.setZero(range); }

  void addSlice(nupic::UInt32 which, nupic::UInt32 src, nupic::UInt32 dst) {
    tensor_.addSlice(which, src, dst);
  }

  PySparseTensor factorMultiply(const TIV &dims,
                                const PySparseTensor &B) const {
    return factorMultiply(PyTensorIndex(dims), B);
  }

  PySparseTensor factorMultiply(const PyTensorIndex &dims,
                                const PySparseTensor &B) const {
    PySparseTensor C(getBounds());
    tensor_.factor_apply_fast(dims, B.tensor_, C.tensor_,
                              std::multiplies<nupic::Real>());
    return C;
  }

  PySparseTensor outerProduct(const PySparseTensor &B) const {
    PySparseTensor C(PyTensorIndex(getBounds(), B.getBounds()));
    tensor_.outer_product_nz(B.tensor_, C.tensor_,
                             std::multiplies<nupic::Real>());
    return C;
  }

  PySparseTensor innerProduct(const nupic::UInt32 dim1,
                              const nupic::UInt32 dim2,
                              const PySparseTensor &B) const {
    // Only works on rank 2 tensors right now
    if ((getRank() != 2) || (B.getRank() != 2))
      throw std::invalid_argument(
          "innerProduct only works for rank 2 tensors.");
    PySparseTensor C(PyTensorIndex(getBound(1 - dim1), B.getBound(1 - dim2)));
    tensor_.inner_product_nz(dim1, dim2, B.tensor_, C.tensor_,
                             std::multiplies<nupic::Real>(),
                             std::plus<nupic::Real>());
    return C;
  }
  ...
  PySparseTensor factorAdd(const PyTensorIndex &dims,
                           const PySparseTensor &B) const {
    PySparseTensor C(getBounds());
    tensor_.factor_apply_nz(dims, B.tensor_, C.tensor_,
                            std::plus<nupic::Real>(), true);
    return C;
  }
  ...
  PySparseTensor marginalize(const PyTensorIndex &dims) const {
    ...
    tensor_.accumulate_nz(dims, B.tensor_, std::plus<nupic::Real>(),
                          nupic::Real(0.0f));
    return B;
  }
