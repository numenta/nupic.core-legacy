...
public:
  ...
  PyBindTensorIndex getLowerBound() const
  {
    PyBindTensorIndex bounds(rank(), (const nupic::UInt32 *) 0);
    getLB(bounds);
    return bounds;
  }

  PyBindTensorIndex getUpperBound() const
  {
    PyBindTensorIndex bounds(rank(), (const nupic::UInt32 *) 0);
    getUB(bounds);
    return bounds;
  ...
  PyBindTensorIndex getDimensions() const
  {
    PyBindTensorIndex bounds(rank(), (const nupic::UInt32 *) 0);
    getDims(bounds);
    return bounds;
  ...
  PyBindTensorIndex getOpenDimensions() const
  {
    PyBindTensorIndex bounds(getNumOpenDims(), (const nupic::UInt32 *) 0);
    getOpenDims(bounds);
    return bounds;
  }

  //  PyBindTensorIndex getSliceBounds(const TIV &maxBounds) const
  PyBindTensorIndex getSliceBounds() const
  {
    PyBindTensorIndex bounds(getNumOpenDims(), (const nupic::UInt32 *) 0);
    nupic::UInt32 n = rank();
    nupic::UInt32 cur = 0;
...
//{
//  nupic::SparseTensor<PyBindTensorIndex, nupic::Real> tensor_;
//  
//public:
//  PySparseTensor(const std::string &state);
//  PySparseTensor(const TIV &bounds) : tensor_(PyBindTensorIndex(bounds)) {}
//  PySparseTensor(const PyBindTensorIndex &bounds) : tensor_(bounds) {}
//  PySparseTensor(/*const*/ PyObject *dense);
//
//  nupic::UInt32 getRank() const { return tensor_.getRank(); }
//
//  PyBindTensorIndex getBounds() const { return tensor_.getBounds(); }
//  nupic::UInt32 getBound(const nupic::UInt32 dim) const { return tensor_.getBound(dim); }
//
//  nupic::Real get(const TIV &i) const { return get(PyBindTensorIndex(i)); }
//  nupic::Real get(const PyBindTensorIndex &i) const { return tensor_.get(i); }
//  void set(const TIV &i, nupic::Real x) { set(PyBindTensorIndex(i), x); }
//  void set(const PyBindTensorIndex &i, nupic::Real x) { tensor_.set(i, x); }
//  void set(const TIV &i, PyObject *x) { set(PyBindTensorIndex(i), x); }
//  void set(const PyBindTensorIndex &i, PyObject *x);
//
...
//  { tensor_.resize(PyBindTensorIndex(dims)); }
//
//  void resize(const PyBindTensorIndex &dims)
//  { tensor_.resize(dims); }
//
...
//  {
//    PyBindTensorIndex dims = range.getSliceBounds();
//    PySparseTensor t(dims);
...
//  { return factorMultiply(PyBindTensorIndex(dims), B); }
//
//  PySparseTensor factorMultiply(const PyBindTensorIndex &dims, const PySparseTensor &B) const
//  {
...
//  {
//    PySparseTensor C(PyBindTensorIndex(getBounds(), B.getBounds()));
...
//    PySparseTensor C(PyBindTensorIndex(getBound(1-dim1),B.getBound(1-dim2)));
...
//  { return factorAdd(PyBindTensorIndex(dims), B); }
//
//  PySparseTensor factorAdd(const PyBindTensorIndex& dims, const PySparseTensor& B) const
//  {
//    PySparseTensor C(getBounds());
...
//  }
//
//  PySparseTensor getComplementBounds(const PyBindTensorIndex &dims) const
//  {
//    PyBindTensorIndex process(tensor_.getBounds());
//    nupic::UInt32 n = dims.size();
//    for(nupic::UInt32 i=0; i<n; ++i)
//      process[dims[i]] = 0;
//
//    n = process.size();
//    PyBindTensorIndex remain(n - dims.size(), (const nupic::UInt32 *) 0);
//    nupic::UInt32 cur = 0;
//    for(nupic::UInt32 i=0; i<n; ++i) {
...
//  { return marginalize(PyBindTensorIndex(dims)); }
//
//  PySparseTensor marginalize(const PyBindTensorIndex &dims) const
//  {
...
//  }
//  
//  PyBindTensorIndex argmax() const;
//  nupic::Real max() const;
//
//  PySparseTensor max(const TIV &dims) const
//  { return this->max(PyBindTensorIndex(dims)); }
//
//  PySparseTensor max(const PyBindTensorIndex &dims) const
//  {
//    PySparseTensor B(getComplementBounds(dims));
...
//    std::vector<PyBindTensorIndex> ind(nnz);
