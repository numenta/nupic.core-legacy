...
from htm_advanced.algorithms.monitor_mixin.metric import Metric
from htm_advanced.algorithms.monitor_mixin.monitor_mixin_base import MonitorMixinBase
from prettytable import PrettyTable

from htm_advanced import (IndicesTrace, CountsTrace, BoolsTrace, StringsTrace)
...
class TemporalMemoryMonitorMixin(MonitorMixinBase):
    ...
    def mmGetMetricSequencesPredictedActiveCellsPerColumn(self):
        ...
        for predictedActiveCells in (list(self._mmData["predictedActiveCellsForSequence"].values())):
    ...
    def mmGetMetricSequencesPredictedActiveCellsShared(self):
        ...
        return Metric(self, "# sequences each predicted => active cells appears in", list(numSequencesForCell.values()))
    ...
    def mmPrettyPrintConnections(self):
        ...
        for column in columns:
            ...
            for cell in cells:
                ...
                text += ("Column {0:3} / Cell {1:3}:\t({2}) {3}\n".format( column, cell, len(list(segmentDict.values())), "[{0}]".format(",             ".join(list(segmentDict.values())))))
    ...
    def mmPrettyPrintSequenceCellRepresentations(self, sortby="Column"):
        ...
        for sequenceLabel, predictedActiveCells in (iter(self._mmData["predictedActiveCellsForSequence"].items())):
            cellsForColumn = self.mapCellsToColumns(predictedActiveCells)
        ...
        return ([Metric.createFromTrace(trace, excludeResets=resetsTrace)
                        ...
                        [Metric.createFromTrace(trace)
                            for trace in self.mmGetDefaultTraces()[-3:-1]] 
                                + [self.mmGetMetricSequencesPredictedActiveCellsPerColumn(), self.mmGetMetricSequencesPredictedActiveCellsShared()])
