...
from htm_advanced.frameworks.location.path_integration_union_narrowing import computeRatModuleParametersFromReadoutResolution
...
class GridCellLocationRegionTest(unittest.TestCase):
    ...
    def testPathIntegration(self):
        ...
        # Move up 10
        Motors.addDataToQueue(motor, [0, 10])
        net.run(1)
        ...
        Motors.addDataToQueue(motor, [0, -5])
        Motors.addDataToQueue(motor, [0, -5])
        net.run(2)
    ...
    def testLearning(self):
        ...
        for objectDescription in OBJECTS:
            ...
            for iFeature, feature in enumerate(objectDescription["features"]):
                ...
                Sensors.addDataToQueue(sensor, FEATURE_ACTIVE_COLUMNS[featureName], reset, 0)
                Sensors.addDataToQueue(candidates, FEATURE_CANDIDATE_SDR[featureName], reset, 0)
                net.run(1)
                reset = False
