...
class CoordinateEncoderTest(unittest.TestCase):
    ...
    def testEncode(self):
        ...
        with self.assertRaises(AssertionError):
            ...
            encoder.encode((coordinate, float(radius)), output)
    ...
    def testEncodeRelativePositions(self):
        ...
        overlaps = overlapsForRelativeAreas(999, 51, np.array([100, 200]), 10, dPosition=np.array([2, 2]), num=5)
        self.assertDecreasingOverlaps(overlaps)


    def testEncodeRelativeRadii(self):
        # As radius increases, the overlap should decrease
        overlaps = overlapsForRelativeAreas(999, 25, np.array([100, 200]), 5, dRadius=2, num=5)
        self.assertDecreasingOverlaps(overlaps)

        # As radius decreases, the overlap should decrease
        overlaps = overlapsForRelativeAreas(999, 51, np.array([100, 200]), 20, dRadius=-2, num=5)
        self.assertDecreasingOverlaps(overlaps)


    def testEncodeRelativePositionsAndRadii(self):
        # As radius increases and positions change, the overlap should decrease
        overlaps = overlapsForRelativeAreas(999, 25, np.array([100, 200]), 5, dPosition=np.array([1, 1]), dRadius=1, num=5)
        self.assertDecreasingOverlaps(overlaps)
    ...
    def testEncodeAdjacentPositions(self, verbose=False):
        ...
        for i in range(repetitions):
            overlaps = overlapsForRelativeAreas(n, w, np.array([i * 10, i * 10]), radius, dPosition=np.array([0, 1]), num=1)
...
def encode(encoder, coordinate, radius):
    output = SDR(encoder.getWidth())
    encoder.encode((coordinate, radius), output)
    return output.dense
